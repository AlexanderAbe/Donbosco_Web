...
const ThieuNhiModel = {
    ...
    async updateStudent(idTn, yearId, data) {
        ...
        try {
            ...
            const assignment = await client.query(`
                ...
                LIMIT 1
            `, [idTn, yearId]);
            if (!assignment.rows.length) throw new Error('Không tìm thấy phân lớp của thiếu nhi trong niên khóa này.');
            if (!String(data.ten || '').trim()) throw new Error('Tên thiếu nhi không được để trống.');
            if (!validGenders.includes(data.gioi_tinh)) throw new Error('Giới tính thiếu nhi không hợp lệ.');
            ...
            await client.query('UPDATE PHAN_LOP SET trang_thai = $1 WHERE id_phan_lop = $2', [data.trang_thai, assignment.rows[0].id_phan_lop]);
