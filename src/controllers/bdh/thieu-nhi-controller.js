...
const sacramentColumns = [
    ...
    ['Xưng tội & Rước lễ', ['NGÀY XƯNG TỘI RƯỚC LỄ', 'NGÀY RƯỚC LỄ', 'NGAY XUNG TOI RUOC LE', 'NGAY RUOC LE']],
    ['Thêm sức', ['NGÀY THÊM SỨC', 'NGAY THEM SUC']]
];

const getCell = (row, names) => {
    const key = Object.keys(row).find(item => names.includes(item.trim().toUpperCase()));
    return key ? row[key] : '';
};
...
const parseImportDate = value => {
    ...
    if (/^\d{8}$/.test(compact)) return `${compact.slice(4)}-${compact.slice(2, 4)}-${compact.slice(0, 2)}`;
...
const ThieuNhiController = {
    async getTrangQuanLy(req, res) {
        try {
            ...
            const gender = validGenders.includes(req.query.gioi_tinh) ? req.query.gioi_tinh : '';
            const search = String(req.query.search || '').trim();
            const limit = Math.min(Math.max(getPositiveInt(req.query.limit) || 20, 10), 100);
            ...
            const [khoiList, lopList, result] = await Promise.all([
                ...
                yearId
                    ? ThieuNhiModel.getPageData({ yearId, khoiId, lopId, gender, search, page: requestedPage, limit })
    ...
    async getDetail(req, res) {
        try {
            ...
            if (!idTn) return res.status(400).json({ error: 'Mã thiếu nhi không hợp lệ.' });
            const detail = await ThieuNhiModel.getDetail(idTn, yearId);
            if (!detail) return res.status(404).json({ error: 'Không tìm thấy thiếu nhi.' });
            res.json(detail);
        } catch (error) {
    ...
    async importExcel(req, res) {
        try {
            ...
            for (const [index, row] of sheetData.entries()) {
                ...
                const ngaySinh = parseImportDate(getCell(row, ['NGÀY SINH', 'NGAY SINH']));
                ...
                for (let parentNumber = 1; parentNumber <= 2; parentNumber++) {
                    const sdt = text(getCell(row, [`SĐT PHỤ HUYNH ${parentNumber}`, `SDT PHU HUYNH ${parentNumber}`]));
                    const tenPhuHuynh = text(getCell(row, [`TÊN PHỤ HUYNH ${parentNumber}`, `TEN PHU HUYNH ${parentNumber}`]));
                    const tenThanhPhuHuynh = text(getCell(row, [`TÊN THÁNH PH ${parentNumber}`, `TEN THANH PH ${parentNumber}`]));
                    const moiQuanHe = text(getCell(row, [`QUAN HỆ PH ${parentNumber}`, `QUAN HE PH ${parentNumber}`])) || 'Cha/Mẹ';
                    if (sdt || tenPhuHuynh || tenThanhPhuHuynh) item.phuHuynh.push({ sdt, tenPhuHuynh, tenThanhPhuHuynh, moiQuanHe });
                }

                for (const [loaiBiTich, columns] of sacramentColumns) {
                    const rawDate = getCell(row, columns);
                    if (rawDate !== '') item.biTich.push({ loaiBiTich, ngayLanhNhan: parseImportDate(rawDate) });
                }
